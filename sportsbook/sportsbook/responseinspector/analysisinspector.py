...
class AnalysisInspector:
    ...
    def init_xpath(self, season):
        if int(season) < 2013:
            standing_div_id = "//div[@style='display:block']"
            home_table = standing_div_id + self.first_table + self.second_row + self.first_column + self.first_table
        else:
        ...
        guest_table = standing_div_id + self.first_table + self.first_row + self.second_column + self.first_table
        ...
        guest_last_6match_row = guest_total_row + self.sixth_row
        # 主队的积分，排名，胜率，近六场的胜平负
